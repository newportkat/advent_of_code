const findNiceStrings = (string) => {
  ...
  console.log(doubleLetters.test("jhmomnrbfaawicea"))
  ...
  console.log(`The total number of nice strings is: ${totalNiceStrings}`)
}

findNiceStrings(`uxcplgxnkwbdwhrp
suerykeptdsutidb
dmrtgdkaimrrwmej
ztxhjwllrckhakut
gdnzurjbbwmgayrg
...
const containsThreeVowels = (string) => {
  const totalVowels = string.match(/[aeiou]/gi)
  const doubleLetters = /a(?=a)|b(?=b)|c(?=c)|d(?=d)/gi
  const exclusions = /a(?=b)|c(?=d)|p(?=q)|x(?=y)/gi

  console.log(totalVowels ? totalVowels.length : 0)
  console.log(doubleLetters.test(string))
  console.log(exclusions.test(string))
}
