...
const modifyToggleInstruction = (string) => {
  const toggleCheck = /toggle/
  if (toggleCheck.test(string)) {
    return `turn ${string}`
  } else {
    return string
  }
...
const createCoordinateKey = (x, y) => {
  //  return { x, y }
  return `${x},${y}`
}

const followLightInstructions = (santasInstructions) => {
  // Separate long challenge input into indivudal instruction strings
  const arrayOfStrings = santasInstructions.split(/\r?\n/)
  let modifiedArrayOfStrings = arrayOfStrings.map((instruction) =>
    modifyToggleInstruction(instruction)
  )
  let instructionObjects = []

  // Create objects from instructions
  for (let i = 0; i < modifiedArrayOfStrings.length; i++) {
    let instruction = modifiedArrayOfStrings[i].split(" ")
    let operator = instruction[1]
    let x1 = parseInt(instruction[2].split(",")[0], 10)
    let y1 = parseInt(instruction[2].split(",")[1], 10)
    let x2 = parseInt(instruction[4].split(",")[0], 10)
    let y2 = parseInt(instruction[4].split(",")[1], 10)
  ...
  // Return how many lights are turned on in total
  let lightMap = new Map()

  //Outer loop for each individual set of instructions
  for (let i = 0; i < instructionObjects.length; i++) {
    // Inner loop for on (increase brightness by 1)
    if (instructionObjects[i].operator === "on") {
      for (
        let j = instructionObjects[i].x1;
        j <= instructionObjects[i].x2;
        j++
      ) {
        for (
          let k = instructionObjects[i].y1;
          k <= instructionObjects[i].y2;
          k++
        ) {
          let key = createCoordinateKey(j, k)
          if (lightMap.has(key)) {
            let newValue = lightMap.get(key) + 1
            lightMap.set(key, newValue)
          } else {
            lightMap.set(key, 1)
          }
        }
      }
      // Inner loop for off (decrease brightness by 1)
    } else if (instructionObjects[i].operator === "off") {
      for (
        let j = instructionObjects[i].x1;
        j <= instructionObjects[i].x2;
        j++
        ...
        for (
          ...
          let key = createCoordinateKey(j, k)
          if (lightMap.has(key) && lightMap.get(key) > 0) {
            //If light exists, minus one from existing value associated with key
            let newValue = lightMap.get(key) - 1
            lightMap.set(key, newValue)
          }
        }
      }
      // Inner loop for toggle (increase by 2 or create with 2 as initial brightness level)
    } else {
      for (
        let j = instructionObjects[i].x1;
        j <= instructionObjects[i].x2;
        ...
        for (
          ...
          let key = createCoordinateKey(j, k)
          if (lightMap.has(key)) {
            let newValue = lightMap.get(key) + 2
            lightMap.set(key, newValue)
          } else {
            lightMap.set(key, 2)
          }
  ...
  let mapString = ""
  lightMap.forEach((value, key) => {
    mapString += `${key} => ${value}\n`
  })

  // Write to a file
  fs.writeFileSync("lightMapOutput.txt", mapString)

  let totalBrightness = 0
  for (let value of lightMap.values()) {
...
followLightInstructions(`turn on 489,959 through 759,964
